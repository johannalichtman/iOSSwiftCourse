...
typealias Velocity = Double // call Double Velocity in this playground
...
extension Car {
    enum CarKind: Printable {                                       // conforms to Printable protocol
        case Coupe, Sedan
    ...
    var kind: CarKind {                                             // computed property
        if numberOfDoors == 2 {
            return .Coupe
        } else {
            return .Sedan
